...
public class MotionPlanner{
	...
	private void expandGrassfire(MotionPlannerState state){		
		...
		ArrayList <MotionPlannerState> neighbors = null;
		...
		for(MotionPlannerState neighbor : neighbors){
			...
			if (motionPlannerContext.costMap.isInCollision(new Point2D.Double(neighbor.x(), neighbor.y()))) {
				continue;
			}
			// Step 3.2: Make sure the neighbor node is not already on open or closed before adding it to open.
			if (motionPlannerContext.statesOpened.contains(neighbor) || motionPlannerContext.statesClosed.contains(neighbor)) {
				continue;
			}
	...
	private MotionPlannerState getBestHorizonNode() {
	    ...
	    for (Iterator<MotionPlannerState> openIt = open.iterator(); openIt.hasNext(); ) {
	    	...
	    	if (open.isEmpty()) {
	    		...
	    		double distance = state.getF(); // fill me in
