...
public class MotionPlannerSearchGraph{
	...
	public ArrayList <MotionPlannerState> generateLegalNeighbors(MotionPlannerState state) {
		...
		if(do8ConnectedTopology){
			int[] ii8 = {x - 1, x, x + 1, x - 1, x + 1, x - 1, x, x + 1}; // TODO: Fix me
			int[] jj8 = {y - 1, y - 1, y - 1, y, y, y + 1, y + 1, y + 1}; // TODO: Fix me
			ii = ii8;
			jj = jj8;
		// Step 2b: For 4 connected case, create two arrays of length 4
		// of x & y integer cell displacements to neighbors
		} else{
			int[] ii4 = {x, x - 1, x + 1, x}; // TODO: Fix me
			int[] jj4 = {y - 1, y, y, y + 1}; // TODO:Fix me
			ii = ii4;
			jj = jj4;
		}
	...
	public Point2D cellCenterInWorld(int i, int j){
		...
		Point2D outRect = new Point2D.Double(i + 0.5, j + 0.5);
		// Step 2: map it inRect coordinates and send it out
		Point2D inRect = rectMap.outRectToInRect(outRect); 
		return new Point2D.Double(inRect.getX(), inRect.getY()); // return worldPoint
	}
	/**
